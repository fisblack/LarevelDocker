...
$(function() {
    ...
    var productSearch = new Bloodhound({
        ...
        limit: 10,
        remote:   {
            url: '/api/product/find?type=order&q=%QUERY%',
            ajax: {
                type: 'GET'
            },
            wildcard: '%QUERY%'
    ...
    $('#addProductList').click(function () {
        if (product) {
            ...
            if (productQty && parseInt(productQty, 10) > 0) {
                products.push({id: product.id, qty: productQty});
                ...
                /**
                 *Update shipping after product added
                 */
                calculateShippingPrice(products);
                clearData();
            }
        }
    });
    ...
    function cllAjax(options) {
        return $.ajax(options)
    }

    function calculateTotalPrice() {
        return priceBeforeDiscount + priceShipping - priceDiscount;
    }

    function calculateShippingPrice(items) {
        cllAjax({
            type: 'GET',
            url: '/api/shipping/price',
            data: {
                type_id: 1,
                region_id: 1,
                products: items
            ...
            priceShipping = parseInt(res['shipping_fee'], 10);
            $('#total_price').text(calculateTotalPrice().format(2));
            $('#shipping_free').text(priceShipping.format(2));
        }).error(function (err) {
            console.log(err);
        })
